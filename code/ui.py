...
class UI:
    ...
    def show_bar(self, current_amount, max_amount, bg_rect, color):
        ...
        stat_ratio = current_amount / max_amount
        current_width = bg_rect.width * stat_ratio

        current_rect = bg_rect.copy()
        current_rect.width = current_width

        # draw the current amount
        pygame.draw.rect(self.display_surface, color, current_rect)
        pygame.draw.rect(self.display_surface, UI_BORDER_COLOR, current_rect, 3)
        
    def show_exp(self, exp):
        text_surf = self.font.render(str(int(exp)), False, TEXT_COLOR)
        text_rect = text_surf.get_rect(bottomright = (self.display_surface.get_width() - 30, self.display_surface.get_height() - 30))
    ...
    def weapon_overlay(self, weapon_index, has_switched):
        bg_rect = self.selecion_box(10, 630, has_switched) # Weapon box
    ...
    def magic_overlay(self, magic_index, has_switched):
        bg_rect = self.selecion_box(80, 635, has_switched)
    ...
    def display(self, player):
        ...
        self.weapon_overlay(player.weapon_index, not player.can_switch_weapon) # Weapon overlay
        self.magic_overlay(player.magic_index, not player.can_switch_magic) # Magic overlay